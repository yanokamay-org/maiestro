...
**Codex sessions get no session color.** For a repo whose agent is Codex (issue #162), the task runs the resolved `codex` binary alone — `'/opt/homebrew/bin/codex'`, labelled "Start Codex". Codex has no `--name` or `/color`, and any initial prompt would start a real model turn, so the session itself carries neither the name nor the color. Everything else still applies: the popover row and VS Code's title/status/activity bars are themed with the worktree's color exactly as for Claude, and `window.title`, the terminal font, and teardown's window matching don't depend on the agent. The session record stores the agent, so the regenerated task on reopen keeps launching the record's agent: the one the worktree was spawned with, unless the user switched it from the row's command strip (see *Switching a session's agent* below).

**Antigravity sessions get no session color either.** For a repo whose agent is Antigravity (issue #185), the task runs the resolved `agy` binary alone, labelled "Start Antigravity". `agy` has no session-name flag and no `/color`, and an initial prompt (`-i`) would start a real model turn, so the same rules as Codex apply. The bars, popover row and window title are still themed. Its status hooks live in the worktree's `.agents/hooks.json`, not on the command line (`docs/session-status.md`).
...
## Switching a session's agent

**AI Agent…** in a row's command strip opens a picker listing the available agents (`AgentSwitchDialog`, current one marked, with a short explanation). Choosing another (issue #186, `session_agent::session_set_agent`) changes an existing worktree's agent without re-spawning it. It updates the session record's `agent` and installs the new agent's status hooks (removing our Claude hooks from `.claude/settings.local.json` when leaving Claude, or our `maiestro-status` group from `.agents/hooks.json` when leaving Antigravity, and keeping the user's). It then regenerates the `.vscode` files from the record with `refresh_vscode_files`, so the switch never re-themes the worktree: the color, emoji, title, and branch stay the same. The old agent's conversation does not carry over.

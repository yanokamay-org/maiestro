...
Instead, starting work creates the worktree and **launches the repo's agent (`claude`, `codex` or `agy`) with the right command-line arguments into a real, user-facing session** — either a VSCode window or a standalone terminal (selected per-repo). The conversation lives in that terminal/editor and the user interacts with it directly.
